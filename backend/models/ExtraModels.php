...
class AvaliacaoModel extends BaseModel
{
    ...
    public function upsert(int $usuarioId, string $tipo, int $conteudoId, int $nota): void
    {
        $stmt = $this->db->prepare("
            INSERT INTO avaliacoes (usuario_id, conteudo_tipo, conteudo_id, nota)
            VALUES (?,?,?,?)
            ON DUPLICATE KEY UPDATE nota=VALUES(nota), atualizado_em=NOW()
        ");
        $stmt->execute([$usuarioId, $tipo, $conteudoId, $nota]);
        $this->recalcMedia($tipo, $conteudoId);
    }

    public function userNote(int $usuarioId, string $tipo, int $conteudoId): ?int
    {
        $stmt = $this->db->prepare("SELECT nota FROM avaliacoes WHERE usuario_id=? AND conteudo_tipo=? AND conteudo_id=? LIMIT 1");
    ...
    {
        $stmt = $this->db->prepare("SELECT AVG(nota) AS media, COUNT(*) AS total FROM avaliacoes WHERE conteudo_tipo=? AND conteudo_id=?");
    ...
    {
        $tbl = match($tipo) { 'filme'=>'filmes','serie'=>'series','anime'=>'animes', default=>'filmes' };
        $this->db->prepare("
            UPDATE $tbl SET
...
class AnuncioModel extends BaseModel
{
    ...
    {
        $stmt=$this->db->prepare("INSERT INTO anuncios (nome,codigo_html,posicao,tipo_conteudo,status,prioridade,data_inicio,data_fim,criado_por) VALUES (?,?,?,?,?,?,?,?,?)");
        $stmt->execute([$data['nome'],$data['codigo_html'],$data['posicao'],$data['tipo_conteudo']??'todos',$data['status']??1,$data['prioridade']??5,$data['data_inicio']??null,$data['data_fim']??null,$data['criado_por']??null]);
...
class AcessoModel extends BaseModel
{
    ...
    {
        $stmt=$this->db->prepare("INSERT INTO acessos (conteudo_tipo,conteudo_id,usuario_id,ip,user_agent,referer) VALUES (?,?,?,?,?,?)");
        $stmt->execute([$data['tipo']??'outro',$data['conteudo_id']??null,$data['usuario_id']??null,$data['ip']??null,isset($data['ua'])?substr($data['ua'],0,490):null,isset($data['referer'])?substr($data['referer'],0,490):null]);
    }
    ...
    {
        $stmt=$this->db->prepare("SELECT DATE(criado_em) AS dia, COUNT(*) AS total FROM acessos WHERE criado_em>=DATE_SUB(NOW(),INTERVAL ? DAY) GROUP BY dia ORDER BY dia ASC");
        $stmt->execute([$dias]);
        return $stmt->fetchAll();
    }
}
...
class ProgressoModel extends BaseModel
{
    ...
    public function upsert(int $usuarioId, string $tipo, int $conteudoId, int $posicaoSeg, ?int $duracaoSeg=null): void
    {
        ...
        $this->db->prepare("
            ...
            VALUES (?,?,?,?,?,?)
            ON DUPLICATE KEY UPDATE posicao_seg=VALUES(posicao_seg),duracao_seg=VALUES(duracao_seg),concluido=VALUES(concluido)
    ...
    {
        $stmt=$this->db->prepare("SELECT * FROM progresso WHERE usuario_id=? AND concluido=0 ORDER BY atualizado_em DESC LIMIT ?");
