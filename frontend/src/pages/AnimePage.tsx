...
import { animeService, episodioService, comentarioService, avaliacaoService } from '@/services'
import { RatingButton, ReviewList, CommentForm, CommentList, AdBlock } from '@/components/ui/index'
import type { Anime, Temporada, Episodio, Avaliacao } from '@/types'
...
export default function AnimePage() {
  ...
  const loadComments = useCallback(() => {
    if (!anime) return
    comentarioService.list('anime', anime.id).then(({ data }) => setComments(data.data ?? [])).catch(() => {})
  }, [anime])
  ...
  const loadReviews = useCallback(() => {
    ...
    avaliacaoService.get('anime', anime.id, 1, 20)
      .then(({ data }) => {
        ...
        <div className="max-w-6xl mx-auto px-4 -mt-24 relative z-10 pb-16">
          <div className="flex flex-col md:flex-row gap-8 mb-10">
            ...
            <div className="flex-1">
              ...
              {anime.titulo_original && <p className="text-gray-500 text-sm mb-2">{anime.titulo_original}</p>}
              ...
              <div>
                ...
                <RatingButton tipo="anime" conteudoId={anime.id}
                ...
                {episodios.map(ep => (
                  <Link key={ep.id} to={`/episodio/${ep.id}`}
                    ...
                    {ep.thumbnail_url ? (
                      ...
                      <div className="w-28 h-16 bg-dark-700 rounded flex items-center justify-center flex-shrink-0 text-gray-500 group-hover:text-orange-400 transition-colors text-xl">▶</div>
                    )}
          ...
          <div className="mt-12">
            ...
            <div className="mt-4">
              <CommentForm conteudoTipo="anime" conteudoId={anime.id} onSuccess={loadComments} />
